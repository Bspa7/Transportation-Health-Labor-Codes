...
# Crear una lista completa de fechas mensuales
fecha_min <- min(main_base$monthly_date)
fecha_max <- max(main_base$monthly_date)
fechas_completas <- tibble(monthly_date = seq(from = fecha_min, to = fecha_max, by = "month"))
...
# Asegurar que todos los meses estén presentes en los resultados
results_unique_monthly <- fechas_completas %>%
  left_join(results_unique_monthly, by = "monthly_date") %>%
  replace_na(list(
    total = 0,
    male = 0,
    female = 0,
    ...
    score_3540 = 0,
    score_4045 = 0,
    mujeres_25_55_score_2025 = 0,
    hombres_25_55_score_2025 = 0
    # Incluir aquí más grupos de puntaje y combinaciones si es necesario
  ))
# ----
...
# 7. Unique Reports by age --------------------------------------------------------

# Definir las edades de pensión para hombres y mujeres
edad_pension_hombres <- 62
edad_pension_mujeres <- 57

# Función para contar personas que cumplen con los criterios de edad de pensión
calcular_pension <- function(data, var_name, edad_pension) {
  data %>%
    filter(!!sym(var_name) >= edad_pension) %>%
    group_by(monthly_date) %>%
    summarise(
      total = n_distinct(personabasicaid),  # Total de personas únicas
      male = n_distinct(personabasicaid[genero == 1]),  # Hombres
      female = n_distinct(personabasicaid[genero == 0]),  # Mujeres
      edad_18_24 = n_distinct(personabasicaid[edad_mensual >= 18 & edad_mensual < 25]),  # Grupo de edad 18-24
      edad_25_55 = n_distinct(personabasicaid[edad_mensual >= 25 & edad_mensual <= 55]),  # Grupo de edad 25-55
      edad_55_mas = n_distinct(personabasicaid[edad_mensual > 55]),  # Grupo de edad 55+
      score_2025 = n_distinct(personabasicaid[puntaje >= 20 & puntaje < 25]),  # Puntaje entre 20 y 25
      score_2530 = n_distinct(personabasicaid[puntaje >= 25 & puntaje < 30]),  # Puntaje entre 25 y 30
      score_3035 = n_distinct(personabasicaid[puntaje >= 30 & puntaje < 35]),  # Puntaje entre 30 y 35
      score_3540 = n_distinct(personabasicaid[puntaje >= 35 & puntaje < 40]),  # Puntaje entre 35 y 40
      score_4045 = n_distinct(personabasicaid[puntaje >= 40 & puntaje <= 45]),  # Puntaje entre 40 y 45
      mujeres_25_55_score_2025 = n_distinct(personabasicaid[genero == 0 & edad_mensual >= 25 & edad_mensual <= 55 & puntaje >= 20 & puntaje < 25]),  # Mujeres 25-55 años y puntaje 20-25
      hombres_25_55_score_2025 = n_distinct(personabasicaid[genero == 1 & edad_mensual >= 25 & edad_mensual <= 55 & puntaje >= 20 & puntaje < 25]),  # Hombres 25-55 años y puntaje 20-25
      # Agregar más combinaciones según sea necesario
    ) %>%
    rename_with(~paste0("pension_", var_name, "_", .), -monthly_date)
}
...
# Inicializar el dataframe de resultados
results_pension_monthly <- data.frame()

# Iteración sobre variables de conteo de personas únicas
for (var in variables_unicos) {
  # Calcular para mujeres y hombres
  temp_df_mujeres <- calcular_pension(main_base, var, edad_pension_mujeres)
  temp_df_hombres <- calcular_pension(main_base, var, edad_pension_hombres)
  
  # Unir resultados
  temp_df <- bind_rows(temp_df_mujeres, temp_df_hombres) %>%
    mutate(genero = ifelse(edad_mensual >= edad_pension_mujeres, "female", "male"))
  
  if (nrow(results_pension_monthly) == 0) {
    results_pension_monthly <- temp_df
  } else {
    results_pension_monthly <- left_join(results_pension_monthly, temp_df, by = "monthly_date")
  }
}

# Asegurar que todos los meses estén presentes en los resultados
fechas_completas <- tibble(monthly_date = seq(from = min(main_base$monthly_date), to = max(main_base$monthly_date), by = "month"))
results_pension_monthly <- fechas_completas %>%
  left_join(results_pension_monthly, by = "monthly_date") %>%
  replace_na(list(
    total = 0,
    male = 0,
    female = 0,

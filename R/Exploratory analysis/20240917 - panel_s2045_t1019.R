...
write_parquet(rips_est_balanced, file.path(data_dir, "panel_rips_s2045_t1014.parquet"))
...
pila_est_balanced <- pila_est_balanced %>%
  mutate(across(c(n_registros_pila, d_registros_pila, ibc_salud_max, ibc_ccf_max, ibc_pens_max, ibc_rprof_max,
                ...
                ~ replace_na(., 0)))
write_parquet(pila_est_balanced, file.path(data_dir, "panel_pila_s2045_t1014.parquet"))
...
gc()
write_parquet(main_base, file.path(data_dir, "panel_total_sample2045_t1014.parquet"))
...
write_parquet(rips_est_balanced, file.path(data_dir, "panel_rips_s2045_t1519.parquet"))
...
pila_est_balanced <- pila_est_balanced %>%
  mutate(across(c(n_registros_pila, d_registros_pila, ibc_salud_max, ibc_ccf_max, ibc_pens_max, ibc_rprof_max,
                  income_base_max, income_base_sum, sal_dias_cot_sum, sal_dias_cot_max, pila_indep, pila_depen),
                ~ replace_na(., 0)))
write_parquet(pila_est_balanced, file.path(data_dir, "panel_pila_s2045_t1519.parquet"))
...
rm(master, pila_est_balanced, rips_est_balanced, skeleton, unique_pbid)
gc()
write_parquet(main_base, file.path(data_dir, "panel_total_sample2045_t1519.parquet"))
...
sample_t1014 <- open_dataset(sprintf('%s/%s', data_dir, 'panel_total_sample2045_t1014.parquet')) %>% collect
sample_t1520 <- open_dataset(sprintf('%s/%s', data_dir, 'panel_total_sample2045_t1519.parquet')) %>% collect

sample_t1019 <- bind_rows(sample_t1014, sample_t1520)

write_parquet(sample_t1019, file.path(data_dir, "panel_total_sample2045_t1019.parquet"))
